...
**Reading the dashboard:** in the live table, the route icon shows what kind of request it was. `(i)` marks a request Claude Code made on its own, not something you typed. The next column is "tier · skill", and the model column shows the model and effort level.
...
| 2 | `/design-review Move sessions to Redis` | `🔖 invoked` · **complex · design-review** · Opus 5.5 · high · **10m left** | The skill's SHA-256 hash matches its entry in `catalog.yaml`, and that entry assigns the complex tier. This starts the session's 10-minute "sticky" timer. |
...
| 5 | `/commit-message Fixed a typo`, then a follow-up | `🔖 invoked` on **light · commit-message**, then `📌 sticky` on light | A newly invoked registered skill sets the session's tier, even if that's lower than before. Stickiness follows the most recent skill. |
| 6 | `/personal-notes check redis ttl`, then a follow-up | `🔹 unregistered` on **light · personal-notes**, then `⚪ untagged` on light with no timer | A skill not in the catalog gets no tier of its own and ends the sticky route. Unknown code never gets an expensive model. |
