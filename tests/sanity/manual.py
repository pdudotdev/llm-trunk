...
| 10 | After test 2 or 5, leave the session idle for more than 10 minutes, then send a message | An `⏳ expired` row appears just before your message; its note says when the route really ended (`… ended at HH:MM (idle)`). Your message shows `⚪ untagged` on **light**. | The sticky tier times out after 10 minutes idle, or 30 minutes after the last invocation. Expiry is recorded at the next request, so that's when the row appears. |
