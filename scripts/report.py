...
from watch import EVENT_RE, REPO, STAMP_RE, load_models  # noqa: E402
...
def summarize(events) -> dict:
    lanes = defaultdict(lambda: {"requests": 0, "input": 0, "cached": 0, "output": 0, "cost": 0.0})
    ...
    for when, kind, event in events:
        ...
        if kind != "spend":
            continue
        lane = lanes[event.get("skill_id") or "untagged"]
...
def render(summary: dict, models: dict[str, str]) -> str:
    if summary["first"] is None:
        return "No llm-trunk events in the log for this period."
    total_cost = sum(lane["cost"] for lane in summary["lanes"].values())
    out = [
        ...
        "",
        f"{'LANE':<24}{'MODEL':>11}{'REQS':>7}{'INPUT':>8}{'CACHED':>8}{'OUTPUT':>8}{'COST':>10}{'SHARE':>7}",
    ]
    for name, lane in sorted(summary["lanes"].items(), key=lambda item: -item[1]["cost"]):
        model = models.get(name, models.get("untagged", "?") if name == "untagged" else "?")
        out.append(
            f"{name:<24}{model:>11}{lane['requests']:>7}{_tokens(lane['input']):>8}"
            f"{_share(lane['cached'], lane['input']):>8}{_tokens(lane['output']):>8}"
            f"{'$' + format(lane['cost'], '.3f'):>10}{_share(lane['cost'], total_cost):>7}"
        )
    requests = sum(lane["requests"] for lane in summary["lanes"].values())
    out.append(f"{'TOTAL':<24}{'':>11}{requests:>7}{'':>8}{'':>8}{'':>8}{'$' + format(total_cost, '.3f'):>10}")
    ...
    if summary["background"]:
        ...
        out += [
            "",
            f"Claude Code background calls: {kinds} — ${summary['background_cost']:.3f} "
            f"({_share(summary['background_cost'], total_cost)} of spend)",
        ]
    if summary["denies"]:
...
def main() -> None:
    ...
    print(render(summarize(parse(result.stdout.splitlines())), load_models()))
