...
| | Auto mode's permission checks | The tier running the model Claude Code asked for, so the safety check isn't weakened (never above what the key could reach with a skill); Claude Code's own effort, `max_tokens` ≤ 8,192 |
...
- [x] **permission check** — auto mode's safety classifier (its security-monitor system prompt, no tools). First, so a check made on a subagent's behalf isn't demoted with it
...
- [x] **background** — session titles, suggestions and recaps, recognized by the fixed text Claude Code sends
...
- **Price:** a cache read costs **0.1×** the normal input price (0.05× on Opus 5.5); the first write costs **1.25×**. So Opus 5.5 and Sonnet 5 cost the same per cached token ($0.20 per million) — moving heavily cached traffic from one to the other saves little; the difference is in fresh input, cache writes and output
...
| [`catalog.yaml`](catalog.yaml) | The routing table: tier `order` (cheapest first, explicit so reordering the file can't invert routing), each tier's effort and caps, and the registered skills with their hash and tier. |
...
| [`scripts/create_usage_key.sh`](scripts/create_usage_key.sh) | Mints a virtual key (`llm-trunk-usage`) with a budget cap (`max_budget`/`budget_duration`). No `models` restriction — LiteLLM checks it against the client's original model, before the callback rewrites it, so it would block every request. Per-skill access is `allowed_skills` in key metadata. |
...
| [`policy/models.py`](policy/models.py) | `model_key()`: any spelling of a Claude model id (dated, `[1m]`, Bedrock-style) → its family. Shared by the callback and the scripts, so both read model ids the same way. |
